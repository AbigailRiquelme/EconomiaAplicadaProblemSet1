...
* Primero obtenemos los nombres de todas las variables de nuestra base, 
* exceptuando la variable "village_id"

ds village_id, not

* Generamos una nueva base de datos que contiene el promedio de cada variable 
*por village_id
...
* Generamos la variable "income_gr", esta le asigna un número del 0 (menor) al 
*4 (mayor) a cada village según su posición en la distribución del ingreso:
...
* Generamos la variable "sub_gr", esta le asigna un número del 0 (menor) al 4 
*(mayor) a cada village según su posición en la distribución de la tasa de 
...
* Decidimos realizar una tabla que compare las medias del subsidio recibido para 
* cada uno de los valores tomados por la variable income_gr, es decir, para cada
* uno de los grupos de ingresos:

tabstat subsidy_rate, stats() by(income_gr)

* También consideramos realizar una tabla que muestre la cantidad de 
* observaciones correspondientes a los distintos grupos de tasas de subsidios 
* para los grupos de ingresos, es decir, la frecuencia de los distintos valores 
* que toma la variable "sub_gr" para cada uno de los valores que toma la 
* variable "income_gr"
...
* Para realizar un análisis más visual consideramos relevante realizar un 
* gráfico de dispersión entre la tasa de subsidio que reciebn las village y el 
* ingreso:
...
* También es informativo realizar un gráfico de barras de la tasa de subsidio 
* media para cada uno de los valores que toma la variable "income_gr":
...
* Observamos que la correlación es significatica al 1%, debido a que p valor 
* asociado es de 0.0000.
...
* A continuación calculamos la correlación entre la tasa de subsidio y las 
* variables relacionadas a las características de cada village: 
...
* Queremos ver si la tasa de subsidio es mayor para las aldeas tratadas 
* (aquellas aldeas que formaban parte del programa CGVO). 

* testeamos que la diferencia entre mean(0)-mean(1) --> Ha: diff < 0, porque 
* sería que mean(tratadas)>mean(no tratadas)

*grafico de medias 

graph bar subsidy_rate, by(cgvo_occur) scheme(mrc) ytitle(Tasa de subsidio (promedio)) bar (1, color(emerald))

* realizamos un ttest para la comparación de medias según si fueron tratados o 
* no 

ttest subsidy_rate, by(cgvo_occur)

* dado que la hipótesis nula del test postula igualdad de medias podemos decir 
* que no hay diferencias significativas entre las medias de subsidios para 
* grupos tratados y no tratados decido a que el p valor que obtenemos es de 
* Pr(T < t) = 0.2254
